...
// Add similar PUT and DELETE routes for consultations
...
app.get('/patients/:id/vaccinations', async (req, res) => {
  try {
    const { id } = req.params;
    const sql = 'SELECT * FROM vaccinations WHERE patient_id = ? ORDER BY date_administered DESC';
    const results = await executeQuery(db, sql, [id]);
    res.json(results);
  } catch (err) {
...
// In your backend route, ensure you're not returning null values
...
// PUT - Update existing vaccination
// In your server.js
...
app.post('/patients/:id/screening-tests', async (req, res) => {
  try {
    ...
    console.log('Received screening test data:', req.body); // Add this line
...
// In your server.js billing POST endpoint
